...
- **Branch:** `feat/bayesian-comparison-notebook`, not yet committed.
- **Session ended with Ryan's local `FAST_MODE=False` run in progress, not confirmed complete.** Don't assume it finished cleanly — check in first.

## Next slice

0. **Confirm Ryan's local `FAST_MODE = False` run of `notebooks/05_bayesian_comparison.ipynb` actually completed** (was still running when Ryan logged off end of session 16, after the arviz/pymc pin fix above). If it didn't get run, or hit another error, sort that out first. Once it has run cleanly (convergence checks print "passed", zero divergences, `r_hat` close to 1.0), get the resulting `narrowing_pct` numbers per channel from Ryan so `docs/guide.html` Section 6 (`#chart10` data + `fig-cap` text) can be updated to cite them instead of the old uncommitted-script figures.
0b. **Rewrite notebook 05's commentary into reader voice before committing** (caught by Ryan, 2026-07-08, mid-run — first-draft markdown was build-log voice, not practitioner voice). Specific fixes needed:
   - Drop the framing that this notebook exists to "backfill a `guide.html` citation because a one-off script was never committed" — that's project-internal history, meaningless and slightly unprofessional to an outside reader. Just state what the notebook investigates (does a Bayesian fit actually fix collinearity?) and let it stand on its own, same as notebooks 01–04.
   - Remove the direct `session 14` reference in the "averaging over noise draws" markdown — describe the single-draw-is-unreliable lesson as a fact about the method, not as a citation to an internal session log.
   - Remove the closing instruction telling the reader to go update `docs/guide.html` — that's a maintainer TODO, not notebook content. Keep any "update the guide" reminder in NOTES.md only.
   - `import pandas as pd` is stuck mid-notebook in the results cell instead of the top imports cell — move it up with the rest.
   - General rule going forward: first-draft notebook markdown apparently still slips into internal-voice by default (matches the existing "internal voice first, practitioner voice rewrite" pattern in the Documentation strategy section above) — don't treat a notebook as finished until it's been read once specifically checking for this, not just for correctness.
1. **Once merged, pull the real numbers forward into `docs/cmo.html`** — LinkedIn sales-hook page (item 2 of the session 14 list). Now has more to draw on than originally scoped: the 3-channel corr=0.7 toy example and plan-year spend totals from notebook 01/02, plus real CAC/ROI figures from notebooks 02–04 (phasing before/after, the 1-year-mark grounding point, and the N=15 realistic-scale example) — all traceable to committed notebook cells rather than invented for the page.
