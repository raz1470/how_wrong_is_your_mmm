"""Add annulus+balanced +/-80% to the bias/variance check."""
...
from how_wrong_is_your_mmm import (
    calibrate_baseline, fit_ols, simulate_demand, simulate_sales, simulate_spend,
)
...
N_SIMS, N_PHASING_SEEDS, N_DEMAND_SEEDS = 40, 6, 6
...
def fit_draws(spend, demand_series, calibration, n_sims):
    ...
    for sim in range(n_sims):
        sales = simulate_sales(spend, TRUE_MR, base_sales=calibration.baseline_level, seed=sim,
...
def main():
    ...
    ref_bias, ref_cv = 60.51, 21.18  # unphased, from the main notebook-11 run
